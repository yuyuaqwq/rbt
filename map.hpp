...
* 而空间开销，btree无人能出其右。
    - btree > art > rb
* 
* 而btree相较于rbtree的缺点是删除时会使其它迭代器失效。
* 
* 我实现的rbtree，由于不使用父节点，通过栈来保存路径，如果保留父节点，会使每个节点的索引空间开销变为12bytes(如果是8bytes的K/V类型，还会导致对齐的额外开销)
* 虽然依旧优于常规标准库实现，但总归是有些遗憾。
* 鉴于迭代器失效的影响并不会太大，故不选择保留父节点。
*/
